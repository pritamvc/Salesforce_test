...
import getPincodeRecord from '@salesforce/apex/LoanApplicationFormHelper.getPincodeRecord';//Not required to change
import getCoAppRecords from '@salesforce/apex/DealLeadCoapplicantEmploymentController.getCoapp';//Change and working
import saveCoApplicant from '@salesforce/apex/DealLeadCoapplicantEmploymentController.saveCoApplicant';//Not completely change
import deleteCoApplicant from '@salesforce/apex/DealLeadCoapplicantEmploymentController.deleteCoApplicant';//Apex class changes
import getApplicantAccoutId from '@salesforce/apex/DealLeadCoapplicantEmploymentController.getAccountIdFromCoApplicant';//Change
import updateLeadTotalPercentage from '@salesforce/apex/DocumentVerification.updateLeadTotalPercentage';//Not required 
import duplicateAccount from '@salesforce/apex/LeadCoapplicantEmploymentController.duplicateAccount';//Not required to change
import getSectionWeightage from '@salesforce/apex/LoanApplicationFormHelper.getSectionWeightage';//Not required to change
import getCheck from '@salesforce/apex/LeadCoapplicantEmploymentController.getCheck';//Not change in method. Added condition of checkId before calling method 
import updateCheck from '@salesforce/apex/LeadCoapplicantEmploymentController.updateCheck';//Not change in method. Added condition of checkId before calling method 
...
import checkPinCodeAvailable from '@salesforce/apex/LeadCoapplicantEmploymentController.checkPinCodeAvailable';//Not Required to change
import getPin from '@salesforce/apex/LeadApplicantDetails.getPin';//Not Required to change
import updateKYCAccount from '@salesforce/apex/KarzaKycOcrController.updateKYCAccount';//Not Required to change
...
import getApplicantEmailAndMobile from '@salesforce/apex/DealLeadCoapplicantEmploymentController.getApplicantEmailAndMobile';//change in class
import OtpRequest from '@salesforce/apex/EmailVerification.OtpRequest';//Changes in the class
import OtpRequestMob from '@salesforce/apex/MobileVerification.OtpRequest';//Changes in the class
import OtpVerify from '@salesforce/apex/EmailVerification.verify';//Changes in the class
import OtpVerifyMob from '@salesforce/apex/MobileVerification.OtpVerify';//Changes in the class
import updateCoapplicantRec from '@salesforce/apex/LeadApplicantDetails.updateEmailAndMobileVerified';//Not Required to channge
import getQuestion from '@salesforce/apex/DealLeadCoapplicantEmploymentController.getQuestionFromLead';//Change in class
import updateQuestion from '@salesforce/apex/DealLeadCoapplicantEmploymentController.updateQuestionOnLead';//Change in class
...
export default class CoapplicantGuarantorChild extends LightningElement {
    ...
    QuestionFromLead(){
        ...
        .then(result => {
            if(this.leadRecordId.startsWith('00Q')){
                var questionNoCoApplicant = result.Why_Mother_Father_are_not_Co_Applicants__c;
            }else{
                var questionNoCoApplicant = result.Mother_Father_non_Co_Applicants_reason__c;
            }
            if(questionNoCoApplicant != undefined){
                this.hideshowQuestion = true;
                this.QuesForcoapplicant = questionNoCoApplicant;
    ...
    handleApplicantCategory(event) {
        ...
        if (event.target.name == "ApplicantCategory") {
            ...
            if (event.target.value === 'CoApplicant') {
    ...
    handleSaveCoApplicant(event) {
        ...
        if (this.listOfAccounts.length > 0) {
            ...
            if(this.QuesForcoapplicant == 'Others' && (this.otherReason == undefined || this.otherReason == '')){
        ...
        if (this.listOfAccounts.length > 0) {
            ...
            if(this.QuesForcoapplicant == 'Others' && (this.otherReason == undefined || this.otherReason == '')){
                ...
                saveCoApplicant({ coApplicantData: this.listOfAccounts, leadId: this.leadRecordId })
                    ...
                    .catch(error => {
                        this.isLoading = false;
                        console.log('Error on Save as Next' , JSON.stringify(error))
                        this.handleErrorMessage(error);
                    })
            }
        }
    ...
    setCoApplicantRecord(listOfAccounts) {
        ...
        listOfAccounts.forEach(function (account) {
            try {
                ...
                account.objeAcc['makeGenderdisable'] = account.objeAcc.Gender__c !== undefined ? true : false;
    ...
    async createAadharRecord(responseObj) {
        ...
        console.log('account id:',responseObj.accountId);
