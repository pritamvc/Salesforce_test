...
import creatCommFormLeadRecords from '@salesforce/apex/LeadApplicantDetails.creatUpdateApplicantRecord';
...
import isPassportUploaded from '@salesforce/apex/LeadApplicantDetails.isPassportUploaded';
...
@track mobileStatus;
    ...
    getAllApplicantData() {
        getapplicantData({ leadGetId: this.leadRecordId })
            .then(result => {
                ...
                if(this.wrapperForCommLeadForm.AccRecords.Account__r.Gender__c != null){
                ...
                if(this.wrapperForCommLeadForm.AccRecords.Account__r.Date_of_Birth__c != null){
    ...
    handleAppliSpouseShowHide(event) {
        if(event.target.value==''||event.target.value==undefined){
            ...
            this.calculatePercent = this.calculatePercent + this.increasePercent;
            console.log("true calculatePercent",this.calculatePercent);
        }
    ...
    consentChange(event) {
        ...
        if (event.target.checked == true) {
            ...
            console.log("total per " + this.percentageAll);
        } else {
    ...
    handleSaveLead() {
        ...
        if(this.saleserrorAadharInvalid == true){
            ...
            let AppliAccDataSaveRec = {
                ...
                //Date_of_Birth__c: this.AppliDOB.toISOString().slice(0, 10),
                Id: this.AppliAccID,
                Aadhar_Number__c: this.finalAadharNumberToSave,
                PAN_Number__c: this.PANNumber,
                Voter_Id__c: this.voterId,
...
checkduplicate(event) {
    if(event.target.value==''||event.target.value==undefined){
        ...
        duplicateAccount({ 'duplicateParameter': this.matchParameter, 'duplicateValue': this.matchValue, 'matchDOB': this.AppliDOB })
            ...
            //this.AppliPermanentTaluka = this.duplicateAccountResult.appPermanentAdd.Taluka__c;
  ...
  @track activesectionname='1';
    ...
    dobBlur(event){
        ...
        const minDate = new Date('1900-01-01');
            ...
            if (enteredDate > currentDate) {
                this.showToast("Success!!", "Entered date should not be Greated than current date", "Success")
                event.target.value='';
            } else if (enteredDate < minDate) {
                this.showToast("Success!!", "Entered date should be valid", "Success")
                event.target.value='';
              }else {
                this.AppliDOB = event.target.value;
            }
    ...
    voterFront(event) {
        ...
        const fileName = 'Applicant Voter'+".";
        const docType = 'VOTER';
        this.openFrontfileUpload(event,fileName,docType);
        this.doc7 = true;
        let file = event.target.files[0]
    ...
    openFrontfileUpload(event,fileName,docType){
        ...
        reader.onload = () => {
            ...
            console.log('BEFORE CALLING APEX METTHOD');
            this.isLoading = true;
            KarzaKycOcr({
                ...
                .then(async (result) => {
                    ...
                    if (responseObj.apistatusCode == 101) {
                        ...
                        .then(() => {
                            console.log('matchParameterDuplicat', this.matchParameterDuplicate);
                        ...
                        .then(() => {
                            ...
                            else if(this.voterId != responseObj.apiVoterNumber){
                                if(this.voterId == undefined || this.voterId == ''){
                        ...
                        .then(() => {
                            ...
                            else if(this.Driving_License_Number__c != responseObj.apiDLNumber){
                                if(this.Driving_License_Number__c == undefined || this.Driving_License_Number__c == ''){
    ...
    async setRecord(responseObj){
        ...
        console.log('If consitiom passportUploded' , this.passportUploded);
        ...
        if(responseObj.leadPinCode != undefined){
            ...
            console.log('AppliCurrentPincode' , this.AppliCurrentPincode);
            ...
            try {
                ...
                if (isPinCodeAvailable == true) {
                    ...
                    this.AppliCurrentTaluka = this.AreaPinCodeResult.Area_Name_Taluka__c;
                    this.AppliCurrentAddProof = 'Aadhar Card';
                    if(responseObj.leadhouse != undefined){
    ...
    checkOCRDuplicate(matchParameter, matchValue, LDateOfBirth){
        ...
        console.log('checkOCRDuplicate Method');
        return new Promise((resolve, reject) => {
            duplicateAccount({ 'duplicateParameter': matchParameter, 'duplicateValue': matchValue, 'matchDOB': LDateOfBirth })
                .then((result) => {
                    this.duplicateAccountResult = result;
                    console.log('duplicateAccountResult' , JSON.stringify(this.duplicateAccountResult));
    ...
    getPassportDoc(){
        ...
        .then(result => {
            console.log('passportUploaded===>' ,result);
    ...
    getleadFileNames(){
        getLeadFileNames({leadId: this.leadRecordId})
        .then(result => {
            console.log('got result');
            this.fileNamesLead = result;
            console.log("document names get lead ========>",result);
            for (let i = 0; i < this.fileNamesLead.length; i++) {
                if(this.fileNamesLead[i].includes("Aadhar_card_front_App_1")){
                    console.log('INSIDE ASSHDARRR FILE front');
                    this.appdoc1 = true;
                    ...
                    console.log('aadharBackName' , this.fileNamesLead[i]);
