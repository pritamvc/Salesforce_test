...
import getUniversityNameCourse from '@salesforce/apex/LeadCourseAndAcademics.getUniversityNameCourse';
...
import getInstituteFromUniversity from '@salesforce/apex/LeadCourseAndAcademics.getInstituteFromUniversity';
...
export default class LeadCourseAndAcademics extends LightningElement {
    ...
    apiCourseStream;
     ...
     //Course Sub Stream from SFDC
     @track CourseSubStream;
     @api CourseSubStreamField = 'Course_Sub_Stream__c';
     @track CourseSubStreamLabel;
     @api CourseSubStreamValue;
    ...
    //@track pattern5 = "^(?:[1-5](?:\.\d[0-9]{1,2})?|5(?:\.0{1,2})?|[1-5])$"; //"^(5(\.([0-4][0-9]|50)?)?|[0-9](\.\d{1,2})?)$";/^(?:[1-4](?:\.[0-9])?|5(?:\.0)?)$/
    //@track pattern10 = "^(?:[1-9](?:\.\d[0-9]{1,2})?|10(?:\.0{1,2})?|[1-10])$";//^(10(\.([0-4][0-9]|50)?)?|[0-9](\.\d{1,2})?)$";
    @track pattern5 = "^(?:[1-4](?:\.[0-9]{1,2})?|5(?:\.0{1,2})?)$";
     //"^(5(\.([0-4][0-9]|50)?)?|[0-9](\.\d{1,2})?)$";/^(?:[1-4](?:\.[0-9])?|5(?:\.0)?)$/
    ...
    getObjectData1({ error, data }) {
        if (data) {
            ...
            this.apiCourseSubStream = this.objectName + '.' + this.CourseSubStreamField;
            this.CourseSubStreamLabel = data.fields[this.CourseSubStreamField].label;
    ...
    //Course Sub Stream   
    @wire(getPicklistValues, { recordTypeId: '$recordTypeId', fieldApiName: '$apiCourseSubStream' })
    getPicklistValues15({ error, data }) {
        if (data) {
            this.CourseSubStreamOptions = data.values.map(plValue => {
                return {
                    label: plValue.label,
                    value: plValue.value
                };
    ...
    getCourseEducationaFunction() {
        getCourseEducationaData({ leadGetId: this.leadRecordId })
            .then(result => {
                ...
                this.CourseStreamValue = this.wrapperLoanApplicationFormResult.LeadRecords.Course_Stream__c;
                this.CourseSubStreamValue = this.wrapperLoanApplicationFormResult.LeadRecords.Course_Sub_Stream__c;
                ...
                if (this.wrapperLoanApplicationFormResult.AppliEduDetailsRecord.length > 0) {
                    ...
                    let myNewElement = { Id: randomId, Education_Qualification__c: "", Type_of_Score__c: "", Year_Completed__c: null, Percentage_Marks_CGPA__c: "", CGPA__c: "", School_College_University__c: "", University_Name__c: "", Course_Name__c: "", Course_Stream__c: "", Course_Start_Date__c: null };
    ...
    handlechange(event) {
        ...
        if (event.target.name == "Course Sub Stream") {
            this.CourseSubStreamValue = event.target.value;           
        }
        ...
        if (event.target.name == "courseStartDate") {
            ...
            if (date1 >= currentdate) {
                ...
                if (this.CourseDurationValue != undefined) {
                    ...
                    console.log('courseDurationValue', courseDurationValue);
    ...
    handleQuesForEducationDetails(event) {
        ...
        console.log('this.QuesForEducationDetails:>', this.QuesForEducationDetails)
    }
    ...
    updateValues(event) {
        ...
        if (event.target.name === 'EducationDetails') {
            ...
            if (!pattern.test(fieldValue) && fieldValue != '') {
                ...
                console.log('valur:' + foundelement.CGPAMarksNotValid);
            } else {
    ...
    handleSaveCourseEduSection() {
        ...
        this.listOfEducationalTable.forEach((rec) => {
            if (rec.Education_Qualification__c == '10th') {
                ...
                hasDiploma = true;
            // } else if (rec.Education_Qualification__c == 'Certificate') {
            //     hasCertificate = true;
            }
        });
        ...
        // } else if (this.QuesForEducationDetails == 'Certificate') {
        //     if (!has10th || !hasCertificate || hasDiploma || has12th || hasGraduate || hasPostGraduate) {
        //         eduErrorForCerfiReqTen = true;
        //     }
        ...
        // } else if (eduErrorForCerfiReqTen) {
        //     this.dispatchEvent(
        //         new ShowToastEvent({
        //             title: 'Information!!',
        //             message: 'Add only 10th & Certificate details for highest qualification as Certificate',
        //             variant: 'Info',
        //         }),
        //     );
            ...
            let CourseSecSaveRec = {
                ...
                Course_Sub_Stream__c: this.CourseSubStreamValue,
    ...
    handleCourseDuration(event) {
        ...
        console.log('CourseDurationValue', this.CourseDurationValue);
