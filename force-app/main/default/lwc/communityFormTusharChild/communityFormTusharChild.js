...
import getleadWithApplicantsRecord from '@salesforce/apex/CommunityLeadFormTusharController.getleadWithApplicantsRec';
import getWrapperClassCommFormLists from '@salesforce/apex/CommunityLeadFormTusharController.getWrapperClassCommFormList';
import getReferences from '@salesforce/apex/CommunityLeadFormTusharController.getReferenceList';
import creatCommFormLeadRecords1 from '@salesforce/apex/CommunityLeadFormTusharController.creatCommFormLeadRecord111';
import createReference from '@salesforce/apex/CommunityLeadFormTusharController.createReferenceRecord';
...
export default class CommunityFormTusharChild extends LightningElement {
   ...
   // @track listOfLoanTable;
    ...
    @track refFirstName;@track refLastName;@track refMiddleName;@track refMobileNumber;
    @track refEmailId; @track refLandlineNumber;@track refOccupation;@track refAddress1;
    @track refAddress2;@track refAddress3;@track refCityName;@track refDistrictName;
    @track refStateName;@track refCountryName;@track refLandMarkName;@track refPinCodeNumber
    ...
    handlechange(event) {
        ...
        for (let i = 0; i < this.listOfAssetTable.length; i++) {
            if (this.listOfAssetTable[i].Id === index) {
        ...
        console.log('size of array--' , this.listOfReferenceTable.length);
        //Commented by tushar
        /*for (let i = 0; i < this.listOfReferenceTable.length; i++) {
        ...
        } else if(event.target.name === 'Pin_Code__c'){
            foundelement.Pin_Code__c = event.target.value;
        }
    ...
    //Added by tushar
    @wire(getReferences, {})
    wiredReferences({ error, data }) {
        console.log('reference data =>' + JSON.stringify(data));
        if (data) {
            this.listOfReferenceTable = JSON.parse(JSON.stringify(data));
            ...
            //Added by tushar
                ...
                let myNewElement = { Id: randomId,Reference_First_Name__c: "",Reference_Last_Name__c: "",Reference_Middle_Name__c: "",Mobile_No__c: "",Email_Id__c: "",Landline_No__c: "",
    ...
    handleSaveReference(event){
        ...
        createReference({refRecordList: this.listOfReferenceTable}).then(result => { 
