...
export default class CommunityFormARSChild extends LightningElement {
   ...
   // @track listOfLoanTable;
    ...
    //Updated for financial 
    @track FinancialDetails;
    ...
    createRowFinancial(listOfFinancialTable) {
        ...
        let myNewElement = { Id: randomId,applicantNames:"",Account_Holder_Name__c: "", appliBranch: "", appliBank: "",Account_Number__c: "",Account_Type__c: "",Account_Operational_Since__c: "",IFSC_Code__c:"",MICR_Code__c:""};
        console.log("myNewElement===>" + myNewElement);  
        //this.listOfFinancialTable.push(myNewElement);      
        this.listOfFinancialTable = [...this.listOfFinancialTable, myNewElement];
        console.log("this.listOfFinancialTableaddRow===>" + this.listOfFinancialTable);
    }
    addNewRowFinancial() {
    ...
    handleChangeFinancial(event){
        ...
        if(event.target.name === 'Account_Holder_Name__c'){ 
            ...
            console.log('JSON.stringify:'+JSON.stringify(foundelement.applicantNames));
        }
    }
        ...
        if (data) {
            ...
            try {
                ...
                console.log('datatest2' , data);
                 let newObj=[];
                for (var gg in data) {
                    newObj.push({ value: data[gg].Id,type:data[gg].Type__c,leadId: data[gg].Lead__r.Id,accountId: data[gg].Account__r.Id})
                    console.log("tryyyyy",newObj)
                }
                
                for (var key in data) {
                    // Here key will have index of list of records starting from 0,1,2,....
                    options.push({ label: data[key].Account__r.Name, value: data[key].Id , type:data[key].Type__c,leadId: data[key].Lead__r.Id,accountId: data[key].Account__r.Id});
    ...
    //Handle change for branch selection and fetching bank,ifsc,micr from master data
    handleBranchSelection(event){
        ...
        .then(result => {
            ...
            foundelement.IFSC_Code__c = this.BranchResult.Name;
    ...
    wiredWrapperClassCommFormList1({ error, data }) {
        if (data) {
            ...
            console.log('bank details:'+JSON.stringify(this.wrapperForCommLeadForm.bankAccount));
           ...
           //Change for financial section
            if(this.wrapperForCommLeadForm.bankAccount.length > 0){
                ...
                console.log('Test Data :'+JSON.stringify(this.listOfFinancialTable));
                this.accNameValue = this.listOfFinancialTable[0].Account__r.Name;
            }else{
            //    let listOfEducationalTable = [];
             ...
             let myNewElement = { Id: randomId,Account_Holder_Name__c: "", Account_Number__c: "", Account_Operational_Since__c: "",Account_Type__c: "",Bank_Branch_IFSC__c: "",Name_of_Bank__c: ""};
                ...
                console.log("this.listOfFinancialTable updated===>" + JSON.stringify(this.listOfFinancialTable)); 
    ...
    //To get updated Applicant value
    handleChangeNameBank(event) {
        var foundelement = this.listOfFinancialTable.find(ele => ele.Id == event.target.dataset.id);
        console.log('updated data'+JSON.stringify(foundelement));

        //setValue(event.target.value);
        console.log('Updated Appilcant Name:'+event.target.value);
        for(var i in this.TypeOptions){
            if(event.target.value === this.TypeOptions[i].value){
                //this.typeofapp = this.TypeOptions[i].type
                
                console.log(this.TypeOptions[i].leadId);
                console.log(this.TypeOptions[i].accountId);
                foundelement.Lead__c = this.TypeOptions[i].leadId;
                foundelement.Account__c = this.TypeOptions[i].accountId;
            }
        }
    }
    ...
    handleSaveFinancial(){
        //console.log(res.applicantNames.value);
        ...
        //If record is not present then remove Id 
        this.listOfFinancialTable.forEach(res =>{
            if(!isNaN(res.Id)){
                res.Id = null;
                // res.Lead__c = '00QBi000003nvjNMAQ';
                // res.Account__c = '001Bi0000064WxAIAU';
                //console.log('updated lead id:'+res.Lead__c);
            }
        });
