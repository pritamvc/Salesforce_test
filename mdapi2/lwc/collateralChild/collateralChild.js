...
export default class CollateralChild extends LightningElement {
    ...
    handleAssetCollType(event) {
        ...
        if (event.target.name == 'Collateral_Type__c') {
            ...
            console.log('foundelement.Collateral_Type__c 1===>', foundelement.Collateral_Type__c);
            // foundelement.Name = foundelement.Collateral_Type__c;
        }

        const selectedCollTypeMovable = this.movableCollTypeRecords.find(record => record.CollateralType__c === foundelement.Collateral_Type__c);
        console.log('selectedCollTypeMovable 1===>', selectedCollTypeMovable);
        const selectedCollTypeImmovable = this.immovableCollTypeRecords.find(record => record.CollateralType__c === foundelement.Collateral_Type__c);
        console.log('selectedCollTypeImmovable 1===>', selectedCollTypeImmovable);

        if (selectedCollTypeMovable || selectedCollTypeImmovable) {
            foundelement.ShowCollateralFDBankBal = true;
            console.log('ShowCollateralFDBankBal if 1===>', foundelement.ShowCollateralFDBankBal);
        } else {
            foundelement.ShowCollateralFDBankBal = false;
            console.log('ShowCollateralFDBankBal else 1===>', foundelement.ShowCollateralFDBankBal);
        }

        if (selectedCollTypeImmovable) {
            foundelement.ShowCollateralResiPlotComm = true;
            console.log('ShowCollateralResiPlotComm if 1===>', foundelement.ShowCollateralResiPlotComm);
        } else {
            foundelement.ShowCollateralResiPlotComm = false;
            console.log('ShowCollateralResiPlotComm else 1===>', foundelement.ShowCollateralResiPlotComm);
        }
    }
