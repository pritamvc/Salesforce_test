...
import getleadWithApplicantsRecord from '@salesforce/apex/DealLeadCoapplicantEmploymentController.getleadWithApplicantsRec';//Apex class changes
//import getWrapperClassCommFormLists from '@salesforce/apex/TempControllerSohail.getWrapperClassCommFormList';
import newmetodtocallemployment from '@salesforce/apex/DealLeadCoapplicantEmploymentController.getEmploymentWithAddress';//Apex Class changes
import saveEmployment from '@salesforce/apex/DealLeadCoapplicantEmploymentController.saveEmployment';//Apex class changes
import deleteEmployment from '@salesforce/apex/DealLeadCoapplicantEmploymentController.deleteEmployment';//Change Not Required
import getAccountsFromEmployment from '@salesforce/apex/DealLeadCoapplicantEmploymentController.getAccountsFromEmploymentAndCoApplicants';//Apex class changes
import { ShowToastEvent } from 'lightning/platformShowToastEvent';
import getPincodeRecord from '@salesforce/apex/LoanApplicationFormHelper.getPincodeRecord';//Change Not Required
...
import NATURE_OF_PROFESSION from '@salesforce/schema/Employment_Details__c.NatureofProfession__c';
import NATURE_OF_BUSINESS from '@salesforce/schema/Employment_Details__c.NatureofBusiness__c';
//import SECTOR from '@salesforce/schema/Employment_Details__c.Sector__c';
import updateLeadTotalPercentage from '@salesforce/apex/DocumentVerification.updateLeadTotalPercentage';//LWC Updated for thiis
import getSectionWeightage from '@salesforce/apex/LoanApplicationFormHelper.getSectionWeightage';//LWC Updated for thiis
import getCheck from '@salesforce/apex/LeadCoapplicantEmploymentController.getCheck';//LWC Updated for thiis
import updateEmploymentCheck from '@salesforce/apex/LeadCoapplicantEmploymentController.updateEmploymentCheck';//LWC Updated for thiis
...
export default class LeadEmploymentDetails extends LightningElement {
    ...
    // Nature of Professional Picklist from Employement Details
    @track NatureofProfessionEmpOptions;
    // Nature of Professional Picklist from Employement Details
    @track NatureofBusinessEmpOptions;
    ...
    //Dependant Picklist- For Employment Type to Sector
    @track controllerFieldApiName = 'Employment_Type__c';
    @track dependentFieldApiName = 'Sector__c';   
    dependentPicklist;
    @track finalDependentVal = [];

    //Dependant Picklist- For Sector to Nature of Business
    @track controllerFieldApiNameNOB = 'Sector__c';
    @track dependentFieldApiNameNOB = 'NatureofBusiness__c';   
    dependentPicklistNOB;
    @track finalDependentValNOB = [];
    ...
    @wire(getPicklistValues, { recordTypeId: '$objectInfo.data.defaultRecordTypeId', fieldApiName: NATURE_OF_PROFESSION })
    NatureofProfessionEmpOptions;

    @wire(getPicklistValues, { recordTypeId: '$objectInfo.data.defaultRecordTypeId', fieldApiName: NATURE_OF_BUSINESS })
    NatureofBusinessEmpOptions;

    // @wire(getPicklistValues, { recordTypeId: '$objectInfo.data.defaultRecordTypeId', fieldApiName: SECTOR })
    // SectorEmpOptions;
...
//     @track finalDependentVal = [];
    ...
    //Dependant Picklist- For Employment Type to Sector
    @wire(getPicklistValuesByRecordType, { objectApiName: EMPLOYMENT, recordTypeId: '$objectInfo.data.defaultRecordTypeId' })
    fetchPicklist({ error, data }) {
        console.log('WIRE CHECK SECTOR 1');
        if (data && data.picklistFieldValues) {
            console.log('INSIDE IF WIRE');
            ...
            //console.log('this.controllingPicklist:::', JSON.stringify(this.controllingPicklist));
            this.dependentPicklist = data.picklistFieldValues[this.dependentFieldApiName];
            console.log('this.dependentPicklist:::', JSON.stringify(this.dependentPicklist));
            this.setSectorvalue();
    ...
    //Dependant Picklist- For Sector to Nature of Business
    @wire(getPicklistValuesByRecordType, { objectApiName: EMPLOYMENT, recordTypeId: '$objectInfo.data.defaultRecordTypeId' })
    fetchPicklistNOB({ error, data }) {
        console.log('WIRE CHECK NOB 4');
        if (data && data.picklistFieldValues) {
            console.log('INSIDE IF WIRE');
            console.log('data.picklistFieldValues:::', JSON.stringify(data.picklistFieldValues));            
            this.dependentPicklistNOB = data.picklistFieldValues[this.dependentFieldApiNameNOB];
            console.log('this.dependentFieldApiNameNOB:::', JSON.stringify(this.dependentPicklistNOB));
            this.setNatureOfBusinessvalue();
    ...
    //Dependant Picklist- For Employment Type to Sector for Fetch on Handle of Employment Type
    fetchSectorDependentValue(event) {
    ...
    //Dependant Picklist- For Employment Type to Sector for Fetch on Load called in setSectorvalue method
    fetchSectorDependentValueOnLoad(element) {       
        element.finalDependentVal = [];        
        const selectedVal = element.objEmployment.Employment_Type__c;        
        let controllerValues = this.dependentPicklist.controllerValues;        
        this.dependentPicklist.values.forEach(depVal => {
            depVal.validFor.forEach(depKey => {               
                if (depKey === controllerValues[selectedVal]) {                    
    ...
    //Dependant Picklist- For Employment Type to Sector for call the fetchSectorDependentValueOnLoad method used for Save, Delete and on Load(Page refresh)
    setSectorvalue() {        
        this.listOfEmploymentTable?.forEach(element => {
            this.fetchSectorDependentValueOnLoad(element);            
        });        
    }  

      //Dependant Picklist- For Sector to Nature of Business for Fetch on Handle of Sector
      fetchNatureOfBusinessDependentValue(event) {
        var foundelement = this.listOfEmploymentTable.find(ele => ele.objEmployment.Id == event.target.dataset.id);        
        foundelement.finalDependentValNOB = [];        
        const selectedValNOB =  event.target.value;     
        let controllerValuesNOB = this.dependentPicklistNOB.controllerValues;       
        this.dependentPicklistNOB.values.forEach(depVal => {
            depVal.validFor.forEach(depKey => {               
                if (depKey === controllerValuesNOB[selectedValNOB]) {                                   
                    foundelement.finalDependentValNOB.push({ label: depVal.label, value: depVal.value });
                }
            });
        });       
    }

    //Dependant Picklist- For Sector to Nature of Business for Fetch on Load called in setNatureOfBusinessvalue method
    fetchNatureOfBusinessDependentValueOnLoad(element) {       
        element.finalDependentValNOB = [];        
        const selectedValNOB = element.objEmployment.Sector__c;        
        let controllerValuesNOB = this.dependentPicklistNOB.controllerValues;        
        this.dependentPicklistNOB.values.forEach(depVal => {
            depVal.validFor.forEach(depKey => {               
                if (depKey === controllerValuesNOB[selectedValNOB]) {                    
                    element.finalDependentValNOB.push({ label: depVal.label, value: depVal.value });
                }
            });
        });       
    }

    //Dependant Picklist- For Sector to Nature of Business for call the fetchNatureOfBusinessDependentValueOnLoad method used for Save, Delete and on Load(Page refresh)
    setNatureOfBusinessvalue() {        
        this.listOfEmploymentTable?.forEach(element => {
            this.fetchNatureOfBusinessDependentValueOnLoad(element);            
        });        
    }  

    connectedCallback() {
        console.log('connectedCallback CHECK 4');
    ...
    handleEmploymentTypeSelect(event) {
        console.log('handleEmploymentTypeSelect');
        var foundelement = this.listOfEmploymentTable.find(ele => ele.objEmployment.Id == event.target.dataset.id);
        foundelement.objEmployment.coapplicantStudenterror = false;
        console.log('######## coapplicantStudenterror  ');
        console.log('######## coapplicantStudenterror  ' + foundelement.objEmployment.coapplicantStudenterror);
        if (event.target.name == 'Employment_Type__c') {
            foundelement.objEmployment.Employment_Type__c = event.target.value;
            foundelement.objEmployment.NatureofProfession__c = '';
            ...
            foundelement.objEmployment.NatureofBusiness__c = '';
            ...
            foundelement.objEmployment.Sector__c = '';
        ...
        this.fetchSectorDependentValue(event);
        this.fetchNatureOfBusinessDependentValue(event);
    }
        ...
        for (var i in this.TypeOptions) {
            if (event.target.value === this.TypeOptions[i].value) {
                if(this.leadRecordId.startsWith('00Q')){
                    ...
                    foundelement1.objEmployment.Deal__c = this.TypeOptions[i].dealId;
                }
        ...
        getleadWithApplicantsRecord({ leadGetId: this.leadRecordId })
            .then(result => {
                ...
                for (var key in result) {
                    options.push({ label: result[key].Account__r.Name, value: result[key].Account__c, type: result[key].Type__c, leadId: result[key].Lead__r.Id, isIncome: result[key].Is_Income_Considered_Financial__c, dealId: result[key].Deal__c});
                }
    ...
    handleChangeEmployment(event) {
        ...
        else if (event.target.name === 'Sector__c') {
            foundelement.objEmployment.Sector__c = event.target.value;
            this.fetchNatureOfBusinessDependentValue(event);
        }
        ...
        else if (event.target.name === 'NatureofProfession__c') {
            foundelement.objEmployment.NatureofProfession__c = event.target.value;
        }
        ...
        else if (event.target.name === 'NatureofBusiness__c') {
            foundelement.objEmployment.NatureofBusiness__c = event.target.value;           
        }
    ...
    handleNextEmployment(event) {
        ...
        var natureOfBusiIsNull;
        var natureOfProfessionIsNull;
        ...
        if (this.listOfEmploymentTable.length > 0) {
            for (var i = 0; i < this.listOfEmploymentTable.length; i++) {
                ...
                if (record.objEmployment.applicantIsFinancial == 'Yes' && record.objEmployment.Employment_Type__c == 'SALARIED' &&
                    ...
                    (record.objEmployment.Name_Of_The_Company__c == '' || record.objEmployment.Name_Of_The_Company__c == undefined ||
                        ...
                        record.objEmployment.Total_Professional_Experience__c == '' || record.objEmployment.Total_Professional_Experience__c == undefined || record.objEmployment.Sector__c == '' || record.objEmployment.Sector__c == undefined)) {
                    salariedmissing = true;
                    if (salariedmissing)
                        break;
                }
                else if ((record.objEmployment.Employment_Type__c == 'SELF EMPLOYED PROFESSIONAL(SEP)') &&
                    (record.objEmployment.Total_Professional_Experience__c == '' || record.objEmployment.Total_Professional_Experience__c == undefined || record.objEmployment.Type_Of_Company__c == '' || record.objEmployment.Type_Of_Company__c == undefined || record.objEmployment.Sector__c == '' || record.objEmployment.Sector__c == undefined)) {
                ...
                else if (record.objEmployment.Employment_Type__c == 'SELF EMPLOYED NON PROFESSIONAL(SENP)' && (record.objEmployment.NatureofBusiness__c == null || record.objEmployment.NatureofBusiness__c == '')) {
                    natureOfBusiIsNull = true;
                    if (natureOfBusiIsNull)
                        break;
                }

                else if (record.objEmployment.Employment_Type__c == 'SELF EMPLOYED PROFESSIONAL(SEP)' && (record.objEmployment.NatureofProfession__c == null || record.objEmployment.NatureofProfession__c == '')) {
                    natureOfProfessionIsNull = true;
                    if (natureOfProfessionIsNull)
                        break;
                }
            }
            ...
            } else if (natureOfBusiIsNull == true) {
                this.dispatchEvent(
                    new ShowToastEvent({
                        title: 'Error!!',
                        message: 'Nature of Business is mandatory for SENP',
                        variant: 'Error',
                    }),
                );
            } else if (natureOfProfessionIsNull == true) {
                this.dispatchEvent(
                    new ShowToastEvent({
                        title: 'Error!!',
                        message: 'Nature of Profession is mandatory for SEP',
                        variant: 'Error',
                    }),
                );
            }
