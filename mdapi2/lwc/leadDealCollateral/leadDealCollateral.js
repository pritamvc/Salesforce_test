...
import getleadWithApplicantsRecForAsset from '@salesforce/apex/DealLoanApplicationFormHelper.getleadWithApplicantsRecForAsset';//Apex Classes changes
import getPincodeRecord from '@salesforce/apex/LoanApplicationFormHelper.getPincodeRecord';//Not required to change
import getCollateralAssetList from '@salesforce/apex/DealLoanApplicationFormHelper.getCollateralAssetList';//Apex Classes changes
import createCollateralSave from '@salesforce/apex/DealLoanApplicationFormHelper.createCollateralSave';//Not Required to change
import deleteCollateral from '@salesforce/apex/LoanApplicationFormHelper.deleteCollateral';//Not required to change
import getMovableCollTypeRecords from '@salesforce/apex/LoanApplicationFormHelper.getMovableCollTypeRecords';//Not required to change
import getImmovableCollTypeRecords from '@salesforce/apex/LoanApplicationFormHelper.getImmovableCollTypeRecords';//Not required to change
...
export default class LeadCollateral extends LightningElement {
    @api leadRecordId;
    ...
    getCollateralAssetFunction() {
        console.log('INSIDE getCollateralAssetFunction');
        ...
        getCollateralAssetList({ leadGetId: this.leadRecordId })
            .then(result => {
                ...
                if (result) {
                    ...
                    console.log('listOfCollateralTable-?' , JSON.stringify(this.listOfCollateralTable));
                    console.log('length-?' , this.listOfCollateralTable.length);
                    ...
                    if (this.listOfCollateralTable.length > 0) {
                        ...
                        this.listOfCollateralTable.forEach((collateralAsset) => {
                            try {
                                ...
                                console.log('selectedCollTypeMovable', selectedCollTypeMovable);
                                console.log('selectedCollTypeImmovable', selectedCollTypeImmovable);
                                ...
                                console.log('ShowCollateralFDBankBal==>', collateralAsset.ShowCollateralFDBankBal);
                                ...
                                console.log('ShowCollateralResiPlotComm==>', collateralAsset.ShowCollateralResiPlotComm);
        ...
        for (var i in this.TypeOptionsCollateral) {
            if (event.target.value === this.TypeOptionsCollateral[i].value) {
                if(this.leadRecordId.startsWith('00Q')){
                    ...
                    foundelement1.Deal__c = this.TypeOptionsCollateral[i].dealId;
                }
    ...
    removeTheRowCollateral(event) {
        ...
        if (this.listOfCollateralTable.length > 1) {
            this.listOfCollateralTable.forEach((element, index) => {
                if (element.Id == event.target.dataset.id) {
                    this.listOfCollateralTable.splice(index, 1);
                    return;
                }
            });
        }
    ...
    handleNextCollateral() {
        ...
        if (this.listOfCollateralTable.length > 0) {
            for (var i = 0; i < this.listOfCollateralTable.length; i++) {
                ...
                console.log('selectedCollTypeImmovable ===>',JSON.stringify(selectedCollTypeImmovable));
                console.log('record.Survey_Plot_No__c===>', record.Survey_Plot_No__c);
                ...
                if (record.Collateral_Type__c == '' || record.Collateral_Type__c == undefined || record.AccountId == '' || record.AccountId == undefined) {
                    ...
                    record.Building_House_No__c == '' || record.Building_House_No__c == undefined || record.Floor__c == '' || record.Floor__c == undefined || record.Pin_Code__c == '' || record.Pin_Code__c == undefined)) {
                    error5 = true;
                    break;
                }
                else if ((selectedCollTypeMovable || selectedCollTypeImmovable) && (record.Asset_Value__c < 0 || record.Asset_Value__c == 0)) {
                ...
                else if (record.Collateral_Type__c == 'Land Plot' && (record.Survey_Plot_No__c == undefined || record.Survey_Plot_No__c == '')) {
                    plotNoIsNull = true;
                    break;
                }
            }
            ...
            else if (plotNoIsNull === true) {
                this.dispatchEvent(
                    new ShowToastEvent({
                        title: 'Error!!',
                        message: 'Survey/Plot No is mandatory for Land Plot',
                        variant: 'Error',
                    }),
                );
            }
