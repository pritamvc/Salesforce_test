   ...
   // @track listOfLoanTable;
    ...
    wiredWrapperClassCommFormList1({ error, data }) {
        ...
        if (data!=undefined) {
            ...
            debugger;
            console.log("eeeeeeeeeee========  "+this.wrapperForCommLeadForm.LeadRecords.Id );
            this.leadIDLoan = this.wrapperForCommLeadForm.LeadRecords.Id==undefined?this.wrapperForCommLeadForm.LeadRecords.Id:this.wrapperForCommLeadForm.LeadRecords.Id;
        ...
        // }
        console.log('dataTest1CHILDddddddddd====>' + JSON.stringify(data));
    }
    ...
    openFrontfileUpload(event,fileName) {
        ...
        reader.onload = () => {
            ...
            this.tempName = tempData.filename
            this.fileData.push(tempData);
            console.log(this.fileData)
            let groupedData = this.fileData.reduce((acc, curr) => {
                let fileId = curr.fileId;
                if (!acc[fileId]) {
                    acc[fileId] = [];
                }
                acc[fileId].push(curr);
                return acc;
            }, {});

            let newArray = Object.values(groupedData);
            console.log("NEWWW======",JSON.stringify(newArray));
            this.dispatchEvent(
                new ShowToastEvent({
                    title: 'Success',
    ...
    aadharBack(event) {
        this.openFrontfileUpload(event);
    ...
    handleAccordionClick(event){
        ...
        //     }
        // }
        if(event.detail.openSections==="Applicant"){
            this.toggler=true;
        }else{
            this.toggler=false;
        }
        if(event.detail.openSections==="Co-applicant"){
            this.Cotoggler=true;
        }else{
